...
class ResumePDF(FPDF):
    ...
    def add_header_info(self, basics: Any):
        ...
        link_parts = []
        links_list = getattr(basics, 'links', []) or []
        linkedin = getattr(basics, 'linkedin', None) or (basics.get('linkedin') if isinstance(basics, dict) else None)
        github = getattr(basics, 'github', None) or (basics.get('github') if isinstance(basics, dict) else None)
        website = getattr(basics, 'website', None) or (basics.get('website') if isinstance(basics, dict) else None)
        
        if linkedin: links_list.append(linkedin)
        if github: links_list.append(github)
        if website: links_list.append(website)
    ...
    def _write_text_segment(self, text: str, line_height: float, bullet_indent: float, right_margin: float):
        ...
        for i, word in enumerate(words):
            ...
            if self.get_x() + word_width > right_margin - 5:
                # Line break for wrapped text - use full line height
                self.ln(line_height)
                self.set_x(self.l_margin + 3)  # Align with first line (after bullet)
...
def generate_pdf_to_bytes(resume: Any, include_summary: bool = False) -> bytes:
    ...
    Returns:
        PDF as bytes
    """
    # Convert to dict for unified access
    ...
    basics_dict = resume.get('basics', {})
    basics_obj = type('Basics', (), basics_dict)
    ...
    # Add header with name and contact info
    pdf.add_header_info(basics_obj)
